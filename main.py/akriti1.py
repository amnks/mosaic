...
def predict(image):
    ...
    # plt.imshow(blur)
    kernel = np.ones((10,10), np.uint8)                  #yaha 10 ki jagah 5 kiye
    ...
    no_danda=cropimg[int((0.25)*h):,:]
    cv2.imshow('no_danda', no_danda)
    
    kernel = np.ones((10,10), np.uint8)  #10 ki jagah 8 kiye hai
    img_min = cv2.erode(no_danda, kernel, iterations=1)
    plt.imshow(img_min)
    
        #plt.imshow(thresh1)
    contours2, hierarchy2= cv2.findContours(img_min, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
